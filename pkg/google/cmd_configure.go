...
func (c commandConfigureConfig) Execute() string {
	if *c.AddRemoveOpts.RemoveFlag {
		err := RemoveMemberToGroupID(*c.AddRemoveOpts.Group, *c.AddRemoveOpts.EmailID)
		if err != nil {
			return fmt.Sprintf("[unfold] unable to remove the member: %s", err.Error())
		}
	...
	if err != nil {
		return fmt.Sprintf("[unfold] failed to add member to the given group: %s", err.Error())
	}
